...
namespace Botzinho.Economy
{
    ...
    public static class EconomyImageHelper {
        ...
        public static async Task<string> GerarImagemSaldo(SocketUser user, long wallet, long bank) {
            ...
            canvas.DrawText(user.Username, width/2, 235, new SKPaint { Color = SKColors.White, TextSize = 24, Typeface = SKTypeface.FromFamilyName("Sans-Serif", SKFontStyle.Bold), TextAlign = SKTextAlign.Center, IsAntialias = true });
        ...
        public static async Task<string> GerarImagemRank(SocketGuild guild, List<(ulong UserId, long Total)> top) {
            ...
            var canvas = surface.Canvas; canvas.Clear(new SKColor(12, 10, 20));
            var bold = SKTypeface.FromFamilyName("Sans-Serif", SKFontStyle.Bold);
            canvas.DrawText("Top", 40, 80, new SKPaint { Color = SKColors.White, TextSize = 48, Typeface = bold, IsAntialias = true });
            canvas.DrawText("Coins", 140, 80, new SKPaint { Color = PurpleTheme, TextSize = 48, Typeface = bold, IsAntialias = true });
            using var http = new HttpClient();
            for (int i = 0; i < top.Count; i++) {
                ...
                try {
                    ...
                    canvas.Save(); canvas.ClipPath(path, SKClipOperation.Intersect, true);
                    canvas.DrawBitmap(bmp, avRect); canvas.Restore();
                    canvas.DrawOval(avRect, new SKPaint { Style = SKPaintStyle.Stroke, StrokeWidth = 2, Color = tCol, IsAntialias = true });
                } catch { }
                canvas.DrawText($"{i+1}. {m?.Username ?? "User"}", x + 100, y + 55, new SKPaint { Color = tCol, TextSize = 22, Typeface = bold, IsAntialias = true });
                canvas.DrawText(EconomyHelper.FormatarSaldo(top[i].Total), x + 100, y + 85, new SKPaint { Color = (i+1 <= 3) ? new SKColor(50,50,50) : new SKColor(180, 180, 200), TextSize = 18, IsAntialias = true });
            }
            ...
            _ = Task.Run(async () => {
                try {
                    ...
                    if (content == "zdaily") {
                        ...
                        await msg.Channel.SendFileAsync(p, $"<a:trofeu:1493063952060387479> **Top Ricos Do Servidor**\n<:emoji_8:1491910148476899529> Você tem **{EconomyHelper.FormatarSaldo(total)}** coins e está em **#{pos}**"); File.Delete(p);
                    }
                        ...
                        EconomyHelper.AdicionarBanco(guildId, alvo.Id, v);
                        await msg.Channel.SendMessageAsync($"<a:lealdade:1493009439522033735> **Sucesso!** Foram adicionados `{EconomyHelper.FormatarSaldo(v)}` cpoints para <:pessoa:1493010183352483840> {alvo.Mention} (Banco).");
                    }
                        ...
                        if (val <= 0 || banco < val || ApostasAtivas.ContainsKey(user.Id)) { await msg.Channel.SendMessageAsync("<:negativo:1492950137587241114> Você não possui **coins** em banco para apostar."); return; }
                        ...
                        if (val <= 0 || banco < val || BlackjackAtivo.ContainsKey(user.Id)) { await msg.Channel.SendMessageAsync("<:negativo:1492950137587241114> Você não possui **coins** em banco para apostar."); return; }
