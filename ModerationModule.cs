...
namespace Botzinho.Moderation
{
    ...
    public class BanModule : InteractionModuleBase<SocketInteractionContext>
    {
            ...
            [Summary("dias", "Dias de mensagens para apagar (0-7)")] int dias = 0)
        {
            ...
            var permissao = AdminModule.VerificarPermissao(Context.Guild.Id, user, "ban");
            ...
            try { await alvo.SendMessageAsync($"tu foi banido de {Context.Guild.Name}"); } catch { }
            await Context.Guild.AddBanAsync(alvo, pruneDays: Math.Clamp(dias, 0, 7), reason: motivo);
            await RespondAsync($"{alvo.Mention} banido\nMotivo: {motivo}");
        }
    }
    ...
    public class UnbanModule : InteractionModuleBase<SocketInteractionContext>
    {
        ...
        public async Task UnbanAsync([Summary("id", "ID do usuário")] string userId)
        {
            ...
            try { await Context.Guild.RemoveBanAsync(id); await RespondAsync($"ID {id} desbanido"); }
    ...
    public class KickModule : InteractionModuleBase<SocketInteractionContext>
    {
        ...
        {
            var user = (SocketGuildUser)Context.User;
            var permissao = AdminModule.VerificarPermissao(Context.Guild.Id, user, "kick");
            ...
            try { await alvo.SendMessageAsync($"tu foi kickado de {Context.Guild.Name}"); } catch { }
            await alvo.KickAsync(motivo);
            await RespondAsync($"{alvo.Mention} tomou kick\nMotivo: {motivo}");
        }
    }
    ...
    public class MuteModule : InteractionModuleBase<SocketInteractionContext>
    {
        ...
        {
            var user = (SocketGuildUser)Context.User;
            var permissao = AdminModule.VerificarPermissao(Context.Guild.Id, user, "mute");
            ...
            { await RespondAsync("duração inválida, usa: 10m, 1h, 1d", ephemeral: true); return; }

            await alvo.SetTimeOutAsync(tempo.Value, new RequestOptions { AuditLogReason = motivo });
            await RespondAsync($"{alvo.Mention} calado por {duracao}\nMotivo: {motivo}");
        }
        ...
        public async Task UnmuteAsync([Summary("usuario", "Usuário")] SocketGuildUser alvo)
        {
            ...
            await RespondAsync($"{alvo.Mention} pode falar de novo");
        }
    }

    public class WarnModule : InteractionModuleBase<SocketInteractionContext>
    {
        [SlashCommand("avisar", "Dá um aviso")]
        public async Task WarnAsync(
            [Summary("usuario", "Usuário")] SocketGuildUser alvo,
            [Summary("motivo", "Motivo")] string motivo = "Sem motivo informado")
        {
            var user = (SocketGuildUser)Context.User;
            var permissao = AdminModule.VerificarPermissao(Context.Guild.Id, user, "warn");
            ...
            if (alvo.IsBot)
            {
                await RespondAsync("não dá pra avisar bot", ephemeral: true);
                return;
            }
            ...
            await RespondAsync($"{alvo.Mention} recebeu um aviso\nMotivo: {motivo}\nTotal: {totalWarns}/3");

            if (totalWarns >= 3)
            {
                try
                {
                    try { await alvo.SendMessageAsync($"você foi banido de {Context.Guild.Name} por acumular 3 avisos"); } catch { }
                    await Context.Guild.AddBanAsync(alvo, reason: "Acumulou 3 avisos.");
                    await Context.Channel.SendMessageAsync($"{alvo.Mention} acumulou 3 avisos e foi banido");
        ...
        public async Task WarnsAsync([Summary("usuario", "Usuário")] SocketGuildUser alvo)
        {
            ...
            while (reader.Read())
            {
                ...
                avisos.Add($"#{warnId} | <@{modId}> | {motivo} | <t:{((DateTimeOffset)DateTime.SpecifyKind(data, DateTimeKind.Utc)).ToUnixTimeSeconds()}:R>");
            }

            if (avisos.Count == 0) { await RespondAsync($"{alvo.Mention} não tem avisos", ephemeral: true); return; }
        ...
        public async Task ClearWarnsAsync([Summary("usuario", "Usuário")] SocketGuildUser alvo)
        {
            ...
            await RespondAsync($"avisos de {alvo.Mention} foram limpos ({deleted} removidos)");
        }
    }
    ...
    public class ChannelModule : InteractionModuleBase<SocketInteractionContext>
    {
        ...
        {
            var user = (SocketGuildUser)Context.User;
            var permissao = AdminModule.VerificarPermissao(Context.Guild.Id, user, "clear");
        ...
        public async Task SlowmodeAsync([Summary("segundos", "Segundos (0 para desativar)")] int segundos)
        {
            ...
            await RespondAsync(segundos > 0 ? $"slowmode de {segundos}s ativado" : "slowmode desativado");
        }
        ...
        {
            var user = (SocketGuildUser)Context.User;
            var permissao = AdminModule.VerificarPermissao(Context.Guild.Id, user, "lock");
            ...
            await RespondAsync("canal trancado");
        }
        ...
        public async Task UnlockAsync()
        {
            ...
            await RespondAsync("canal destrancado");
        }
    }
}
